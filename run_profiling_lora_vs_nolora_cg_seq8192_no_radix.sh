...
# ══════════════════════════════════════════════════════════════
#  Scenario 1: No LoRA + CUDA Graph + Disable Radix Cache
# ══════════════════════════════════════════════════════════════
run_no_lora_cg_no_radix() {
    local PROFILE_DIR="${PROFILE_BASE}/no_lora_cg_no_radix"
    mkdir -p "${PROFILE_DIR}"
    export SGLANG_TORCH_PROFILER_DIR="${PROFILE_DIR}"

    log "═══ Scenario: No LoRA + CUDA Graph + Disable Radix Cache ═══"
    ...
    python -m sglang.bench_serving \
        ...
        --profile \
        --profile-prefix no_lora_cg_no_radix_run1 \
    ...
    python -m sglang.bench_serving \
        ...
        --max-concurrency "$SERVE_MAX_CONCURRENCY" \
        --profile \
        --profile-prefix no_lora_cg_no_radix \
        --profile-output-dir "${PROFILE_DIR}" \
    ...
    log "No-LoRA + CG + No Radix profile (2nd run) saved to: ${PROFILE_DIR}"
}

# ══════════════════════════════════════════════════════════════
#  Scenario 2: LoRA + CUDA Graph (virtual experts) + Disable Radix Cache
# ══════════════════════════════════════════════════════════════
run_lora_cg_no_radix() {
    local PROFILE_DIR="${PROFILE_BASE}/lora_cg_no_radix"
    mkdir -p "${PROFILE_DIR}"
    export SGLANG_TORCH_PROFILER_DIR="${PROFILE_DIR}"

    log "═══ Scenario: LoRA + CUDA Graph (virtual experts) + Disable Radix Cache ═══"
    log "Profile output: ${PROFILE_DIR}"
    ...
    python -m sglang.bench_serving \
        ...
        --lora-name my_lora \
        --profile \
        --profile-prefix lora_cg_no_radix_run1 \
        --profile-output-dir "${PROFILE_DIR}" \
        2>&1 | tee "${PROFILE_DIR}/bench_run1.log"
    ...
    python -m sglang.bench_serving \
        ...
        --lora-name my_lora \
        --profile \
        --profile-prefix lora_cg_no_radix \
        --profile-output-dir "${PROFILE_DIR}" \
        2>&1 | tee "${PROFILE_DIR}/bench_run2.log"
    ...
    log "LoRA + CG + No Radix profile (2nd run) saved to: ${PROFILE_DIR}"
}
...
# ══════════════════════════════════════════════════════════════
echo "================================================================"
echo "  Profiling: LoRA CG vs No-LoRA CG (Radix Cache DISABLED)"
echo "  seq_len=${SERVE_INPUT_LEN}"
...
# --- Scenario 1: No LoRA + CG + No Radix Cache ---
run_no_lora_cg_no_radix
cleanup

# --- Scenario 2: LoRA + CG + No Radix Cache ---
run_lora_cg_no_radix
cleanup
...
for scenario in no_lora_cg_no_radix lora_cg_no_radix; do
