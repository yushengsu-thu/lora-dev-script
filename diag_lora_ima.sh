...
PORT=30000
TP=4
LOG="${SCRIPT_DIR}/diag_lora_nocg_$(date +%Y%m%d_%H%M%S).log"
echo "log -> $LOG"
...
(
  python -m sglang.launch_server \
    ...
    --prefill-attention-backend fa4 --decode-attention-backend fa4 \
...
if ! curl -s -m 3 "http://localhost:${PORT}/health" >/dev/null 2>&1; then
  echo "server NOT ready, tail log:"; tail -120 "$LOG"; exit 1
fi

echo ""
echo "=== bench BS=160, no-cuda-graph, CUDA_LAUNCH_BLOCKING=1 ===" | tee -a "$LOG"
python3 -m sglang.bench_one_batch_server \
    --model None --tokenizer-path "$MODEL_PATH" \
    --base-url "http://localhost:${PORT}/" \
    --batch-size 160 --input-len 8192 --output-len 1024 \
    --result-filename "${SCRIPT_DIR}/diag_lora_result.jsonl" \
